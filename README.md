...
- **`Button` styling prop is `view`, not `variant` or `color`.** Values include `normal`, `action`, `outlined`, `flat`, `raised`, `outlined-info`, `outlined-danger`; size is a separate `size` prop (`s | m | l | xl`).
