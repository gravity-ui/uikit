...
| `@gravity-ui/uikit/legacy`            | `src/legacy.ts`            | Compatibility surface for legacy components; avoid for new work. The `List` there needs the optional peers `react-window` and `react-virtualized-auto-sizer`, the column settings of `Table` and `TableColumnSetup` need `@hello-pangea/dnd` |
