...
Use this page as the human index for repository work. Start with the authoritative documentation,
...
| Prepare a contribution                         | [README development setup](../README.md#development), [contributor overview](../contribute/README.md), [CLA](../CONTRIBUTING.md), and [PR guide](../contribute/pull-request.md) | Only the feature area after the issue/contract is clear                                           | The change-aware route below plus `git diff --check`                                                                        |
| Use a component                                | Its `src/components/<Component>/README.md`; use [layout](layout.md), [theming](theming.md), or [typography](typography.md) when relevant                                        | The component's stories for examples; implementation only if the README is insufficient           | No code checks for a usage answer; verify examples against public imports                                                   |
...
| Change layout or spacing                       | [Layout](layout.md) and [theming](theming.md)                                                                                                                                   | `src/components/layout/`, `styles/themes/common/spacing.scss`, affected component SCSS            | Typecheck/Jest for logic; Stylelint and targeted Playwright for rendering                                                   |
| Change colors, branding, or themes             | [Theming](theming.md) and, when text metrics change, [typography](typography.md)                                                                                                | `styles/themes/`, `styles/styles.scss`, `src/components/theme/`, affected SCSS                    | Stylelint, targeted Playwright, and build for published theme/style changes                                                 |
| Change typography                              | [Typography](typography.md) and [theming](theming.md)                                                                                                                           | `styles/themes/common/typography.scss`, `src/components/Text/`, relevant stories                  | Stylelint, exact Jest tests, and targeted Playwright                                                                        |
| Add or change SSR behavior                     | [SSR guide](server-side-rendering.md) and [architecture](architecture.md#public-package-surface)                                                                                | `src/server.ts` and server-safe implementation such as `src/components/theme/getRootClassName.ts` | ESLint, typecheck, exact Jest tests, full build; keep browser-only code out                                                 |
| Add or change built-in text                    | [I18n guide](i18n.md)                                                                                                                                                           | `src/utils/configure.ts`, `src/i18n/`, and the component's `i18n/` directory                      | ESLint, typecheck, exact component tests; verify all supported locales                                                      |
...
Use the applicable npm scripts declared in `package.json` as the validation entrypoints. These
scripts are the repository source of truth and keep local validation consistent with CI. Do not
replace them with ad hoc shell, Node.js, or other one-off validators. If no package script covers a
desired check, report the validation gap instead of inventing a local check.

- Markdown: `npm run lint:prettier`.
- TS/TSX: run ESLint only on changed files, `npm run typecheck`, and exact co-located Jest files.
  Reserve `--findRelatedTests` for genuinely shared code because a foundational component can pull
  in most of the suite.
