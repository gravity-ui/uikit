...
## Слои всплывающих компонентов

Глобальные уровни наложения используют общую шкалу `--g-layer-*`, которую `styles/styles.css`
задаёт на `.g-root`. Эти токены используются и в стилях компонентов, и в позиционирующих обёртках
React. Текущие уровни модальных окон, панелей и уведомлений сохранены для совместимости со слоями
приложений; верхние уровни вычисляются относительно них.

| Токен               | Где используется                      | Вычисление по умолчанию       | Значение по умолчанию |
| ------------------- | ------------------------------------- | ----------------------------- | --------------------- |
| `--g-layer-drawer`  | `Drawer`                              | `modal - 10`                  | `990`                 |
| `--g-layer-modal`   | `Modal`, включая `Dialog`             | Базовый уровень               | `1000`                |
| `--g-layer-popup`   | `Popup` и использующие его компоненты | `modal`                       | `1000`                |
| `--g-layer-sheet`   | `Sheet`                               | Базовый уровень               | `100000`              |
| `--g-layer-toaster` | `ToasterComponent`                    | `sheet`                       | `100000`              |
| `--g-layer-drag`    | Перетаскиваемые элементы списков      | `max(sheet, toaster) + 1`     | `100001`              |
| `--g-layer-tooltip` | `Tooltip` и `ActionTooltip`           | `max(modal, popup, drag) + 1` | `100002`              |

Для настройки шкалы переопределите токены на `.g-root` после импорта стилей UIKit:
...
Зависимые уровни пересчитываются автоматически. Явные значения `Popup.zIndex` и
`Popup.floatingStyles.zIndex` по-прежнему имеют приоритет над токеном.
Локальные уровни внутри компонентов, например фон кнопки или закреплённые ячейки таблицы,
не относятся к глобальной шкале.

Порталы наследуют токены от своего DOM-контейнера. Задавайте глобальные переопределения на `.g-root`,
чтобы они действовали и на локальные корни темы внутри порталов: переопределение на произвольном
предке якоря не переносится вместе с порталом в `document.body`. Сохраняйте нужный порядок при
переопределении отдельных уровней. Шкала не позволяет выйти из контекстов наложения предков,
обойти обрезку содержимого или перекрыть верхний слой браузера.
