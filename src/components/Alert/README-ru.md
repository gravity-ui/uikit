...
`closeButttonSize` — размер кнопки закрытия. Может быть одним из доступных значений свойства `size` компонента `Button`.
...
| `--g-alert-icon-margin`         | Расстояние между иконкой и контентной частью компонента (текстовая часть + дейтствия)       |
...
| `--g-alert-message-margin`      | Расстояние между заголовком и сообщением.                                                   |
