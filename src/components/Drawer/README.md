...
By default, the `Drawer` slides in and out with a transition. To show or hide it instantly, pass the `disableTransition` prop.
...
| contentOverflow         | Determines whether the `Drawer` has its own scroll indicator inside. By default, it doesn't have                                        |          `visible` `auto`          |    `visible`    |
...
| modal                   | Enables modal focus management                                                                                                          |             `boolean`              |     `true`      |
...
| size                    | Width or height of the drawer in pixels. When the `auto` option is passed, the Drawer's width or height will adjust to fit its content. |         `number \| 'auto'`         |                 |
