...
| itemKey           | Функция, возвращающая уникальный ключ элемента: `(item: any) => string`. Позволяет сохранять состояние компонента элемента при фильтрации или изменении порядка элементов.                                        | `Function`        | индекс элемента       |
