...
| disableAnimation      | Отключает анимации открытия и закрытия при `true` или только указанную фазу при `"open"` или `"close"`. Анимация изменения высоты сохраняется. | `boolean \| "open" \| "close"` |        `false`        |
