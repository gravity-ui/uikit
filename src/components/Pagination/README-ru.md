...
| pageComponent   | Переопределяет корневой элемент кликабельных элементов пагинации (навигационных и постраничных кнопок). Используйте `"a"` для обычных ссылок или компонент роутера.             |  `PaginationPageComponent`  |                       |
