...
| component       | Переопределяет корневой элемент кликабельных элементов пагинации (навигационных и постраничных кнопок). Используйте `"a"` для обычных ссылок или компонент роутера.         |  `React.ElementType`   |                       |
| getItemProps    | Возвращает дополнительные props для каждого кликабельного элемента (например, `href` для `"a"` или `to` для `Link` из роутера). Применяется только когда задан `component`. |       `Function`       |                       |
