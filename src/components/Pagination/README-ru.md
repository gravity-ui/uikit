...
`getItemProps` вызывается для каждого кликабельного элемента (кнопок страниц и навигационных кнопок). Он **не** вызывается для многоточия, индикатора "page of" и для простого (текущего) элемента страницы в мобильной разметке. Pagination-управляемые свойства (`onClick`, `className`, `size`, `view`, `selected`, `disabled`, `qa`, `aria-current`, `extraProps`, `children`) имеют приоритет над значениями, возвращёнными из `getItemProps`.
