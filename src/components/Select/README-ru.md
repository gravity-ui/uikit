...
## Обобщенные типы значений

По умолчанию значения опций — строки, однако `Select` является обобщенным (generic) по типу значения: можно напрямую использовать числа и объекты. Тип значения — второй параметр типа `SelectProps<T, V>` (`V`, по умолчанию `string`); он выводится из `options`, `value` и `onUpdate`:
...
Значения сравниваются по алгоритму SameValueZero (тот же алгоритм используется в `Array.prototype.includes`): числа сравниваются по значению, а объекты — **по ссылке**, то есть элементы `value` должны быть теми же экземплярами, что и соответствующие `options[].value`.

При использовании нестроковых значений учитывайте следующее:

- В формах (со свойством `name`) значения сериализуются при отправке: числа — через `String(value)`, объекты — через `JSON.stringify(value)`.
- Если у опции не заданы `content`, `children` или `text`, отображаемый и фильтруемый текст формируется из сериализованного значения.
- API дочерних элементов `<Select.Option>` не может вывести нестроковый тип значения; в этом случае укажите аргументы типа явно, например `<Select<any, MyValue> ...>`.
...
| defaultValue                                              | Значения по умолчанию для выбранных опций в случае использования неуправляемого состояния. См. [Обобщенные типы значений](#обобщенные-типы-значений). | `V[]`                                    |                                                          |
...
| value                                                     | Значения для выбранных опций, которые передаются в обработчик `onUpdate`. См. [Обобщенные типы значений](#обобщенные-типы-значений).                  | `V[]`                                    |                                                          |
