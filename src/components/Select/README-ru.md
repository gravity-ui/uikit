...
По умолчанию значения опций — строки, но можно напрямую использовать и значения других типов, например числа или объекты. Тип значения задается вторым параметром типа `SelectProps<T, V>` (по умолчанию `V = string`) и автоматически выводится из `options`, `value` и `onUpdate`:
...
Примитивные значения сравниваются по значению, а объекты — **по ссылке**: в `value` должны попадать те же самые экземпляры объектов, что и в `options[].value`, а не их копии.

При работе с нестроковыми значениями учитывайте следующее:

- При отправке формы (если задано свойство `name`) нестроковые значения сериализуются в строку.
- Если у опции не задано ни одно из свойств `content`, `children` и `text`, текст для отображения и фильтрации формируется из значения.
- При использовании дочерних элементов `<Select.Option>` нестроковый тип значения не выводится автоматически — укажите параметры типа явно, например `<Select<any, MyValue> ...>`.
...
| defaultValue                                              | Значения по умолчанию для выбранных опций в случае использования неуправляемого состояния. См. [Обобщенный тип значения](#обобщенный-тип-значения). | `V[]`                                    |                                                          |
...
| value                                                     | Значения для выбранных опций, которые передаются в обработчик `onUpdate`. См. [Обобщенный тип значения](#обобщенный-тип-значения).                  | `V[]`                                    |                                                          |
