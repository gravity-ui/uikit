...
By default, option values are strings; however, `Select` is generic over the value type: numbers and objects can be used directly. The value type is the second type parameter of `SelectProps<T, V>` (`V`, defaults to `string`) and is inferred from `options`, `value`, and `onUpdate`:
...
Values are compared using SameValueZero equality (the same algorithm `Array.prototype.includes` uses): numbers match by value, while objects match **by reference**, meaning the entries of `value` must be the same instances as the corresponding `options[].value`.

Keep in mind the following when using non-string values:

- In forms (with the `name` property), values are serialized for submission: numbers via `String(value)`, objects via `JSON.stringify(value)`.
- If an option has no `content`, `children`, or `text`, the displayed and filtered text falls back to the serialized value.
- The `<Select.Option>` children API cannot infer non-string value types; in this case, provide explicit type arguments, e.g., `<Select<any, MyValue> ...>`.
...
| defaultValue                                              | Default values that represent selected options in case of using an uncontrolled state. See [Generic value types](#generic-value-types) | `V[]`                                    |                                                          |
...
| value                                                     | Values that represent selected options. See [Generic value types](#generic-value-types)                                                | `V[]`                                    |                                                          |
