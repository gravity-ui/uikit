...
By default, option values are strings; however, `Select` is generic over the value type: values of other types — such as numbers or objects — can be used directly. The value type is the second type parameter of `SelectProps<T, V>` (`V`, which defaults to `string`) and is inferred from `options`, `value`, and `onUpdate`:
...
Primitive values are compared by value, while objects are compared **by reference**: the entries of `value` must be the same instances as the corresponding `options[].value`, not copies.

Keep in mind the following when using non-string values:

- In forms (with the `name` property), non-string values are serialized to strings.
- If an option has no `content`, `children`, or `text`, the text used for display and filtering is derived from the value.
- Non-string value types cannot be inferred from `<Select.Option>` children; provide explicit type arguments, e.g., `<Select<any, MyValue> ...>`.
