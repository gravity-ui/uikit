...
| onOpenChange                | Обработчик `(open: boolean, event?: Event, reason?: SheetOpenChangeReason) => void`, как в [`Popup.onOpenChange`](../Popup/README-ru.md#properties).<br>При закрытии запрашивается `open=false` с причиной `escape-key`, `outside-press`, `swipe` или `navigation`.<br>Если передан этот обработчик, шторка остаётся открытой, пока родитель не установит `visible` в `false`. |  `function`   |      `undefined`      |
