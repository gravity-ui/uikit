...
Используйте свойство `size` для задания высоты скелетона (а также ширины для вариантов `'circle'` и `'square'`). Значение по умолчанию — `'m'`. Каждый размер также влияет на токен border-radius для вариантов `'rect'` и `'square'`.
...
| `size`      | Предустановленная высота (и ширина для вариантов `'circle'`/`'square'`). Также управляет токеном border-radius для вариантов `'rect'`/`'square'`. |   `'xs'` \| `'s'` \| `'m'` \| `'l'` \| `'xl'`    |          `m`          |
