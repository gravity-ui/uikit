...
describe('Suggest', () => {
    ...
    test('snapshot: closed state', () => {
        renderSuggest();
        expect(screen.getByTestId('suggest-input').outerHTML).toMatchSnapshot();
    });

    test('snapshot: open popup', async () => {
        const user = userEvent.setup();
        renderSuggest();

        await user.click(screen.getByRole('textbox'));

        expect(screen.getByTestId('suggest-popup').outerHTML).toMatchSnapshot();
        expect(screen.getByTestId('suggest-list').outerHTML).toMatchSnapshot();
    });
});
