...
| onUpdate        | Коллбэк, вызываемый при добавлении принятых файлов                                                                                                                                                           |         `(files: File[]) => void`          |                                            |
| onReject        | Коллбэк, вызываемый при отклонении файлов (неподходящий тип или превышение `maxFilesCount`). Может быть вызван одновременно с `onUpdate` в рамках одного дропа, если часть файлов принята, а часть отклонена | `(items: DropZoneFileRejection[]) => void` |                                            |
...
| multiple        | Булево значение, определяющее, можно ли загружать несколько файлов. Если `false`, допускается только один файл вне зависимости от `maxFilesCount`                                                            |                 `boolean`                  |                                            |
| maxFilesCount   | Максимальное количество файлов (применяется только при `multiple` = `true`). Лишние файлы отклоняются через `onReject`                                                                                       |                  `number`                  |                 `Infinity`                 |
