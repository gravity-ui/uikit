...
именем её секции. Учтите: по контракту композиции `aria-describedby` —
«последний побеждает», собственное значение в overrides `getItemProps` или в
props dnd-адаптера затирает ссылку на секцию.
...
## Активность и её индикация

Активная строка одна (`activeItemId`), а индикация зависит от **модальности
последнего взаимодействия** — «чем пользователь работает сейчас» (модель
`isFocusVisible` react-aria):

- **мышь** (наведение или клик переводят лист в pointer-модальность) —
  активная строка подсвечивается обычным светлым ховером, пока указатель на
  ней; после ухода мыши подсветки нет — тёмный след за мышью не ходит
  (активность и roving tab-stop при этом остаются на строке);
- **клавиатура** — тёмный active-цвет «курсора»; keyboard-модальность
  возвращает любая клавиша, в том числе нажатая вне листа (Tab перед
  tab-in). Программная активация (`activeItemId`/`defaultActiveItemId`)
  показывается так же: начальная модальность — keyboard.

Кастомному рендеру то же самое отдаёт `getItemViewProps()` (проп `active`
вьюхи уже разведён по модальности); свой маркап может читать модальность
напрямую — `ctx.state.activationModality` (`'keyboard' | 'pointer'`, поле
есть только у активной строки), а светлый ховер ему даёт обычный CSS
`:hover`.

`disabled`-строки не участвуют в навигации вовсе (модель React Spectrum):
наведение, клик, стрелки и typeahead их пропускают, фокус на них не
переносится — строка остаётся в списке только семантически
(`aria-disabled`, `data-disabled`).

На время нажатия ядро подавляет выделение текста (модель react-aria:
инлайновый `user-select: none` на нажатой строке, на iOS — на документе):
протяжка мышью, `Shift`+клик в режиме выделения и старт перетаскивания
не создают текстового выделения. В покое строки остаются частью выделения
страницы (Ctrl+A, протяжка, начатая вне списка).
...
### Диапазоны в `multiple`

Жесты диапазона (модель `SelectionManager` react-aria; работают только в
`selectionMode="multiple"` — в `single` Shift игнорируется, жест ведёт себя
как обычный):

- якорь — цель последней не-Shift операции выделения (клик, `Space`,
  `Enter`); обычный жест пере-якоряет, в том числе снимающий выделение;
- `Shift`+клик и `Shift`+`↑`/`↓` заменяют «диапазонную часть» выделения —
  от якоря до цели предыдущего Shift-жеста — диапазоном от якоря до новой
  цели; выделенное вне диапазона не трогается. `Shift`+стрелка двигает
  активность вместе с границей диапазона и не зацикливается на краях
  (обычные стрелки без `Shift` зациклены, как и раньше);
- `Shift`+`Space` — диапазон от якоря до активной строки (при непустом
  typeahead-буфере пробел остаётся частью поиска);
- `Ctrl`/`Cmd`+`A` — выделить все не-`disabled` опции; в
  activedescendant-режиме клавиша не перехватывается (принадлежит инпуту
  владельца фокуса);
- диапазон считается по данным, а не по DOM: под виртуализацией строки
  диапазона за окном тоже выбираются; заголовки секций пропускаются,
  `disabled`-опции жестами не выбираются;
- порядок в `selectedIds`: выделенное вне диапазона сохраняет порядок
  выделения, диапазонная пачка добавляется в конец в порядке данных.

Наружу ничего нового: те же `selectedIds`/`onSelectedUpdate`. Если передан
`onItemAction`, `Shift`+клик и `Shift`+`Space` применяют строку так же, как
обычный жест.
...
с tab-stop) и заголовки секций всегда остаются смонтированными: фокус и
клавиатурная навигация переживают выгрузку строк из окна, а `aria-describedby`
опций не повисает на выгруженном заголовке; на опциях появляются
